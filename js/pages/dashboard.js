import { syncFromKubios, syncTimevarying, getLatestAnalysis, getAnalysisTrend } from '../services/analysisService.js';
...
const RECOMMENDATIONS = {
    good: [
        { title: 'Erinomainen palautuminen', desc: 'Kehosi on palautunut hyvin. Voit jatkaa normaalisti!', type: 'good' },
        { title: 'Stressitaso matala', desc: 'Hermostosi on tasapainossa — hyvä merkki palautumisesta.', type: 'good' }
    ],
    moderate: [
        { title: 'Kohtuullinen palautuminen', desc: 'Huolehdi tauoista ja riittävästä levosta tänään.', type: 'warn' },
        { title: 'Nukkumaanmenoaika', desc: 'Kokeile mennä nukkumaan klo 22–23 — se parantaa palautumista.', type: 'warn' }
    ],
    poor: [
        { title: 'Kehosi tarvitsee lepoa', desc: 'Vältä raskasta kuormitusta tänään ja pyri nukkumaan aiemmin.', type: 'warn' },
        { title: 'Stressitaso koholla', desc: 'Kokeile rentoutumisharjoituksia ennen nukkumaanmenoa.', type: 'warn' }
    ]
};
...
function updateScoreCard(data) {
    ...
    document.getElementById('metricRecovery').textContent = Math.round(readiness) + '%';
...
function renderMiniTrend(trendData) {
    ...
    if (miniTrendRoot) { miniTrendRoot.dispose(); miniTrendRoot = null; }
...
function updateMeters(data) {
    ...
    // Arvot
    document.getElementById('meterReadiness').textContent = Math.round(readiness) + '/100';
    ...
    document.getElementById('meterQuality').textContent   =
        quality === 'GOOD' ? 'Hyvä' :
        quality === 'MODERATE' ? 'Kohtalainen' : 'Heikko';
    ...
    document.getElementById('meterQualityFill').style.width =
        quality === 'GOOD' ? '90%' :
        quality === 'MODERATE' ? '55%' : '25%';
}

function switchView(view) {
    document.getElementById('tabReadiness')?.classList.toggle('active', view === 'readiness');
    document.getElementById('tabTimevarying')?.classList.toggle('active', view === 'timevarying');
    ...
    if (view === 'readiness') {
        ...
        const tv = typeof currentData.timevarying_data === 'string'
        ...
        renderSleepStagesChart('sleepStagesChart', currentData?.sleep_stages ?? null);
    }
}
...
async function init() {
    ...
    try {
        ...
        // Piilota HRV-kortti oletuksena
        const hrvCard = document.getElementById('hrvChartCard');
        if (hrvCard) hrvCard.style.display = 'none';

    } catch (err) {
        console.error('Virhe:', err);
        document.getElementById('scoreRating').textContent = 'Analyysiä ei saatavilla';
    }
}
